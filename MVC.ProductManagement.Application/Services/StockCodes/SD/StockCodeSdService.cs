...
namespace MVC.ProductManagement.Application.Services.StockCodes.SD
{
    public class StockCodeSdService : IStockCodeSdService
    {
        ...
        public StockCodeSdService(AppDbContext db)
        {
            _db = db;
        }

        // ========== ÜRÜN LİSTESİ ==========
        public async Task<List<SdProductDto>> GetSdProductsAsync(CancellationToken ct = default)
        {
            return await _db.SProducts
                .Where(p => p.SProductGroup.Code == "D" && p.Status != Domain.Enums.Status.Deleted)
                .OrderBy(p => p.PrefixIndex)
                .Select(p => new SdProductDto
                {
                    Id = p.Id,
                    Code = p.Code,
                    Name = p.Name
        ...
        public async Task<StockCodeSdFormDto> GetFormDataAsync(Guid productId, CancellationToken ct = default)
        {
            ...
            {
                var feature = new StockCodeSdFormFeatureDto
                {
    ...
    CancellationToken ct = default)
        {
            ...
            // DEFAULT FLUID (SD için)
            var defaultFluid = await _db.Set<Fluid>()
                .FirstOrDefaultAsync(x => x.Code == "D", ct);
        ...
        public async Task<GenericStockCardDetailDto> GetStockCardDetailAsync(Guid stockCardId, CancellationToken cancellationToken = default)
        {
            var stockCard = await _context.Set<StockCard>()
                ...
                ?? throw new InvalidOperationException("SD stok kartı bulunamadı.");
        ...
        // ========== LİSTE ==========
        public async Task<SDStockCardListResultDto> GetStockCardsAsync(
            SDStockCardFilterDto filter,
            CancellationToken ct = default)
        {
            var query = _db.Set<StockCard>()
                .Include(s => s.SProduct)
                .Where(s => s.SProduct.SProductGroup.Code == "D"
            ...
            var items = await query
                ...
                .Select(s => new SDStockCardListItemDto
                {
                    Id = s.Id,
            ...
            return new SDStockCardListResultDto
            {
                Items = items,
        ...
        // ========== DETAY ==========
        public async Task<SDStockCardDetailDto> GetStockCardDetailAsync(
            Guid stockCardId,
            CancellationToken ct = default)
        {
            ...
            return new SDStockCardDetailDto
            {
                Id = stockCard.Id,
                StockCode8 = stockCard.StockCode8,
                Prefix4 = stockCard.Prefix4,
                ...
                CreatedDate = stockCard.CreatedDate,
                CreatedBy = stockCard.CreatedBy,
                FeatureSelections = selections.Select((s, i) => new SDFeatureSelectionDto
                {
