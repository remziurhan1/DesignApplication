...
namespace MVC.ProductManagement.Application.Services.StockCodes.SE
{
    public class StockCodeSeService : IStockCodeSeService
    {
        ...
        public StockCodeSeService(AppDbContext db)
        {
            _db = db;
        }

        // ========== ÜRÜN LİSTESİ ==========
        public async Task<List<SeProductDto>> GetSeProductsAsync(CancellationToken ct = default)
        {
            return await _db.SProducts
                .Where(p => p.SProductGroup.Code == "E" && p.Status != Domain.Enums.Status.Deleted)
                .OrderBy(p => p.PrefixIndex)
                .Select(p => new SeProductDto
                {
                    Id = p.Id,
                    Code = p.Code,
                    Name = p.Name
        ...
        public async Task<StockCodeSeFormDto> GetFormDataAsync(Guid productId, CancellationToken ct = default)
        {
            ...
            var features = new List<StockCodeSeFormFeatureDto>();

            foreach (var rule in productRules)
            {
                var feature = new StockCodeSeFormFeatureDto
                {
                ...
                else
                {
                    ...
                    feature.AvailableValues = sorted
                        .Select(v => new SeFeatureValueOptionDto
                        {
                            Id = v.Id,
                            Code = v.Code,
                            Name = v.Name
    ...
    CancellationToken ct = default)
        {
            ...
            // DEFAULT FLUID (SE için)
            var defaultFluid = await _db.Set<Fluid>()
                .FirstOrDefaultAsync(x => x.Code == "E", ct);
        ...
        public async Task<GenericStockCardDetailDto> GetStockCardDetailAsync(Guid stockCardId, CancellationToken cancellationToken = default)
        {
            var stockCard = await _context.Set<StockCard>()
                ...
                ?? throw new InvalidOperationException("SE stok kartı bulunamadı.");
        ...
        // ========== LİSTE ==========
        public async Task<SEStockCardListResultDto> GetStockCardsAsync(
            SEStockCardFilterDto filter,
            CancellationToken ct = default)
        {
            var query = _db.Set<StockCard>()
                .Include(s => s.SProduct)
                .Where(s => s.SProduct.SProductGroup.Code == "E"
            ...
            var items = await query
                ...
                .Select(s => new SEStockCardListItemDto
                {
                    Id = s.Id,
            ...
            return new SEStockCardListResultDto
            {
                Items = items,
        ...
        // ========== DETAY ==========
        public async Task<SEStockCardDetailDto> GetStockCardDetailAsync(
            Guid stockCardId,
            CancellationToken ct = default)
        {
            ...
            return new SEStockCardDetailDto
            {
                Id = stockCard.Id,
                StockCode8 = stockCard.StockCode8,
                Prefix4 = stockCard.Prefix4,
                ...
                CreatedDate = stockCard.CreatedDate,
                CreatedBy = stockCard.CreatedBy,
                FeatureSelections = selections.Select((s, i) => new SEFeatureSelectionDto
                {
