    ...
    public class EN13458CalculationEngine : ICryogenicsCalculationEngine
    {
        ...
        public Task<EN13458ResultDTO> CalculateAsync(EN13458CalculateDTO input)
        {
            var result = new EN13458ResultDTO
            {
                ...
                WeldLength3000 = input.WeldLength3000,
                CorrosionAllowance = input.CorrosionAllowance,
                BucklingLength = input.BucklingLength,
                ElasticModulus = input.ElasticModulus,
                PoissonRatio = input.PoissonRatio,
                RoundnessErrorPercent = input.RoundnessErrorPercent,
                YieldFactorK = input.YieldFactorK,
                UseGeneralElasticFormula = input.UseGeneralElasticFormula,
                HasStiffener = input.HasStiffener,
                UseManualStiffenerValues = input.UseManualStiffenerValues,
                StiffenerMaterialId = input.StiffenerMaterialId,
                StiffenerMaterialFormId = input.StiffenerMaterialFormId,
                StiffenerInertia = input.StiffenerInertia,
                StiffenerArea = input.StiffenerArea,
