...
namespace MVC.ProductManagement.Presentation.Areas.Sales.Controllers
{
    public class SalesRequestController : SalesBaseController
    {
        ...
        public async Task<IActionResult> Details(Guid id)
        {
            ...
            ViewBag.CanUploadPid = canViewPricing;
            return View(vm);
        }
        ...
        public async Task<IActionResult> UploadDocument(SalesRequestDocumentUploadVm vm)
        {
            ...
            {
                TempData["ErrorMessage"] = "GAD ve Datasheet maliyet analizi ile otomatik üretilir. Manuel yükleme sadece P&ID içindir.";
                return RedirectToAction(nameof(Details), new { id = vm.SalesRequestId });
            }

            var canUploadPid = User.IsInRole("Admin") || await HasSalesPermissionAsync(x => x.CanViewSalesPricing);
            if (!canUploadPid)
            {
                TempData["ErrorMessage"] = "P&ID yükleme yetkisi sadece satış müdüründedir.";
                return RedirectToAction(nameof(Details), new { id = vm.SalesRequestId });
            }
