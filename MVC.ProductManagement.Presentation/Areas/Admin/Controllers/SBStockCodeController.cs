...
namespace MVC.ProductManagement.Presentation.Areas.Admin.Controllers
{
    ...
    public class SBStockCodeController : Controller
    {
        ...
        public SBStockCodeController(
            IStockCodeSbService sbService,
        ...
        public async Task<IActionResult> GetFormData(Guid productId)
        {
            try
            {
                var formData = await _sbService.GetFormDataAsync(productId, CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> Index(SBStockCardFilterDto filter)
        {
            try
            {
                var result = await _sbService.GetStockCardsAsync(filter, CancellationToken.None);
            ...
            catch (Exception ex)
            {
                ...
                return View(new SBStockCardListResultDto());
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            try
            {
                var detail = await _sbService.GetStockCardDetailAsync(id, CancellationToken.None);
                if (detail == null)
                {
                ...
                var viewModel = new SBStockCardDetailViewModel { StockCard = detail };
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            {
                var detail = await _sbService.GetStockCardDetailAsync(id, CancellationToken.None);
                var formData = await _sbService.GetFormDataAsync(detail.ProductId, CancellationToken.None);
                ...
                var updateDto = new SBStockCardUpdateDto
                {
                    StockCardId = id,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, SBStockCardUpdateDto model)
        {
            try
            {
                ...
                await _sbService.UpdateStockCardAsync(model, "Admin", CancellationToken.None);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _sbService.DeleteStockCardAsync(id, "Admin", CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportExcel(SBStockCardFilterDto filter)
        {
            try
            {
                ...
                var listDtos = result.Items.Select(item => new SBStockCardListDto
                {
                    Id = item.Id,
                ...
                var bytes = await _excelService.ExportSBStockCardsAsync(listDtos);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SB_StokKodlari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportDetailExcel(Guid id)
        {
            ...
            {
                var detail = await _sbService.GetStockCardDetailAsync(id, CancellationToken.None);
                var bytes = await _excelService.ExportSBStockCardDetailAsync(detail);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SB_{detail.StockCode8}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Excel export hatası: {ex.Message}";
        ...
        private async Task LoadDatasheetsAsync(Guid stockCardId, SBStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadPricesAsync(Guid stockCardId, SBStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadInventoryAsync(Guid stockCardId, SBStockCardDetailViewModel viewModel)
        {
            try
            {
