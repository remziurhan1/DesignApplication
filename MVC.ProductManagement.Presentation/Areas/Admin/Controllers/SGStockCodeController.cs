...
namespace MVC.ProductManagement.Presentation.Areas.Admin.Controllers
{
    ...
    public class SGStockCodeController : Controller
    {
        ...
        public SGStockCodeController(
            IStockCodeSgService sgService,
        ...
        public async Task<IActionResult> GetFormData(Guid productId)
        {
            try
            {
                var formData = await _sgService.GetFormDataAsync(productId, CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> Index(SGStockCardFilterDto filter)
        {
            try
            {
                var result = await _sgService.GetStockCardsAsync(filter, CancellationToken.None);
            ...
            catch (Exception ex)
            {
                ...
                return View(new SGStockCardListResultDto());
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            try
            {
                var detail = await _sgService.GetStockCardDetailAsync(id, CancellationToken.None);
                if (detail == null)
                {
                ...
                var viewModel = new SGStockCardDetailViewModel { StockCard = detail };
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            {
                var detail = await _sgService.GetStockCardDetailAsync(id, CancellationToken.None);
                var formData = await _sgService.GetFormDataAsync(detail.ProductId, CancellationToken.None);
                ...
                var updateDto = new SGStockCardUpdateDto
                {
                    StockCardId = id,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, SGStockCardUpdateDto model)
        {
            try
            {
                ...
                await _sgService.UpdateStockCardAsync(model, "Admin", CancellationToken.None);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _sgService.DeleteStockCardAsync(id, "Admin", CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportExcel(SGStockCardFilterDto filter)
        {
            try
            {
                ...
                var listDtos = result.Items.Select(item => new SGStockCardListDto
                {
                    Id = item.Id,
                ...
                var bytes = await _excelService.ExportSGStockCardsAsync(listDtos);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SG_StokKodlari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportDetailExcel(Guid id)
        {
            ...
            {
                var detail = await _sgService.GetStockCardDetailAsync(id, CancellationToken.None);
                var bytes = await _excelService.ExportSGStockCardDetailAsync(detail);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SG_{detail.StockCode8}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Excel export hatası: {ex.Message}";
        ...
        private async Task LoadDatasheetsAsync(Guid stockCardId, SGStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadPricesAsync(Guid stockCardId, SGStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadInventoryAsync(Guid stockCardId, SGStockCardDetailViewModel viewModel)
        {
            try
            {
