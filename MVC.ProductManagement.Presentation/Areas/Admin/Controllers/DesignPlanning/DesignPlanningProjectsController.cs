...
[Area("Admin")]
public class DesignPlanningProjectsController : AdminBaseController
{
    private readonly AppDbContext _context;
    private readonly IPlanningService _planningService;
    public DesignPlanningProjectsController(AppDbContext context, IPlanningService planningService) { _context = context; _planningService = planningService; }

    public async Task<IActionResult> Index() => View(await _context.DesignPlanningProjects.Include(x => x.ProjectType).OrderByDescending(x => x.StartDate).ThenBy(x => x.ProjectCode).ToListAsync());
    public async Task<IActionResult> Create() { await LoadProjectTypes(null); return View(new Project { StartDate = DateTime.Today, Priority = 1, Status = ProjectStatus.Waiting }); }

    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(Project project)
    {
        if (!ModelState.IsValid) { await LoadProjectTypes(project.ProjectTypeId); return View(project); }
    ...
    public async Task<IActionResult> Edit(Guid id) { var project = await _context.DesignPlanningProjects.FindAsync(id); if (project == null) return NotFound(); await LoadProjectTypes(project.ProjectTypeId); return View(project); }
    [HttpPost, ValidateAntiForgeryToken]
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Plan(Guid id) { await _planningService.AutoAssignAndPlanAsync(id); return RedirectToAction(nameof(Details), new { id }); }
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        if (project != null) { _context.Remove(project); await _context.SaveChangesAsync(); }
