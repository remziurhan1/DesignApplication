...
namespace MVC.ProductManagement.Presentation.Areas.Admin.Controllers
{
    ...
    public class SHStockCodeController : Controller
    {
        ...
        public SHStockCodeController(
            IStockCodeShService shService,
        ...
        public async Task<IActionResult> GetFormData(Guid productId)
        {
            try
            {
                var formData = await _shService.GetFormDataAsync(productId, CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> Index(SHStockCardFilterDto filter)
        {
            try
            {
                var result = await _shService.GetStockCardsAsync(filter, CancellationToken.None);
            ...
            catch (Exception ex)
            {
                ...
                return View(new SHStockCardListResultDto());
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            try
            {
                var detail = await _shService.GetStockCardDetailAsync(id, CancellationToken.None);
                if (detail == null)
                {
                ...
                var viewModel = new SHStockCardDetailViewModel { StockCard = detail };
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            {
                var detail = await _shService.GetStockCardDetailAsync(id, CancellationToken.None);
                var formData = await _shService.GetFormDataAsync(detail.ProductId, CancellationToken.None);
                ...
                var updateDto = new SHStockCardUpdateDto
                {
                    StockCardId = id,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, SHStockCardUpdateDto model)
        {
            try
            {
                ...
                await _shService.UpdateStockCardAsync(model, "Admin", CancellationToken.None);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _shService.DeleteStockCardAsync(id, "Admin", CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportExcel(SHStockCardFilterDto filter)
        {
            try
            {
                ...
                var listDtos = result.Items.Select(item => new SHStockCardListDto
                {
                    Id = item.Id,
                ...
                var bytes = await _excelService.ExportSHStockCardsAsync(listDtos);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SH_StokKodlari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportDetailExcel(Guid id)
        {
            ...
            {
                var detail = await _shService.GetStockCardDetailAsync(id, CancellationToken.None);
                var bytes = await _excelService.ExportSHStockCardDetailAsync(detail);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SH_{detail.StockCode8}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Excel export hatası: {ex.Message}";
        ...
        private async Task LoadDatasheetsAsync(Guid stockCardId, SHStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadPricesAsync(Guid stockCardId, SHStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadInventoryAsync(Guid stockCardId, SHStockCardDetailViewModel viewModel)
        {
            try
            {
