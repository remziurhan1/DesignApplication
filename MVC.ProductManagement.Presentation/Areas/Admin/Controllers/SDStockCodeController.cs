...
namespace MVC.ProductManagement.Presentation.Areas.Admin.Controllers
{
    ...
    public class SDStockCodeController : Controller
    {
        ...
        public SDStockCodeController(
            IStockCodeSdService sdService,
        ...
        public async Task<IActionResult> GetFormData(Guid productId)
        {
            try
            {
                var formData = await _sdService.GetFormDataAsync(productId, CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> Index(SDStockCardFilterDto filter)
        {
            try
            {
                var result = await _sdService.GetStockCardsAsync(filter, CancellationToken.None);
            ...
            catch (Exception ex)
            {
                ...
                return View(new SDStockCardListResultDto());
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            try
            {
                var detail = await _sdService.GetStockCardDetailAsync(id, CancellationToken.None);
                if (detail == null)
                {
                ...
                var viewModel = new SDStockCardDetailViewModel { StockCard = detail };
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            {
                var detail = await _sdService.GetStockCardDetailAsync(id, CancellationToken.None);
                var formData = await _sdService.GetFormDataAsync(detail.ProductId, CancellationToken.None);
                ...
                var updateDto = new SDStockCardUpdateDto
                {
                    StockCardId = id,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, SDStockCardUpdateDto model)
        {
            try
            {
                ...
                await _sdService.UpdateStockCardAsync(model, "Admin", CancellationToken.None);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _sdService.DeleteStockCardAsync(id, "Admin", CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportExcel(SDStockCardFilterDto filter)
        {
            try
            {
                ...
                var listDtos = result.Items.Select(item => new SDStockCardListDto
                {
                    Id = item.Id,
                ...
                var bytes = await _excelService.ExportSDStockCardsAsync(listDtos);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SD_StokKodlari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportDetailExcel(Guid id)
        {
            ...
            {
                var detail = await _sdService.GetStockCardDetailAsync(id, CancellationToken.None);
                var bytes = await _excelService.ExportSDStockCardDetailAsync(detail);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SD_{detail.StockCode8}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Excel export hatası: {ex.Message}";
        ...
        private async Task LoadDatasheetsAsync(Guid stockCardId, SDStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadPricesAsync(Guid stockCardId, SDStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadInventoryAsync(Guid stockCardId, SDStockCardDetailViewModel viewModel)
        {
            try
            {
