...
namespace MVC.ProductManagement.Presentation.Areas.Admin.Controllers
{
    ...
    public class SEStockCodeController : Controller
    {
        ...
        public SEStockCodeController(
            IStockCodeSeService seService,
        ...
        public async Task<IActionResult> GetFormData(Guid productId)
        {
            try
            {
                var formData = await _seService.GetFormDataAsync(productId, CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> Index(SEStockCardFilterDto filter)
        {
            try
            {
                var result = await _seService.GetStockCardsAsync(filter, CancellationToken.None);
            ...
            catch (Exception ex)
            {
                ...
                return View(new SEStockCardListResultDto());
            }
        }
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            try
            {
                var detail = await _seService.GetStockCardDetailAsync(id, CancellationToken.None);
                if (detail == null)
                {
                ...
                var viewModel = new SEStockCardDetailViewModel { StockCard = detail };
        ...
        public async Task<IActionResult> Edit(Guid id)
        {
            ...
            {
                var detail = await _seService.GetStockCardDetailAsync(id, CancellationToken.None);
                var formData = await _seService.GetFormDataAsync(detail.ProductId, CancellationToken.None);
                ...
                var updateDto = new SEStockCardUpdateDto
                {
                    StockCardId = id,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(Guid id, SEStockCardUpdateDto model)
        {
            try
            {
                ...
                await _seService.UpdateStockCardAsync(model, "Admin", CancellationToken.None);
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _seService.DeleteStockCardAsync(id, "Admin", CancellationToken.None);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportExcel(SEStockCardFilterDto filter)
        {
            try
            {
                ...
                var listDtos = result.Items.Select(item => new SEStockCardListDto
                {
                    Id = item.Id,
                ...
                var bytes = await _excelService.ExportSEStockCardsAsync(listDtos);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SE_StokKodlari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportDetailExcel(Guid id)
        {
            ...
            {
                var detail = await _seService.GetStockCardDetailAsync(id, CancellationToken.None);
                var bytes = await _excelService.ExportSEStockCardDetailAsync(detail);

                return File(bytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"SE_{detail.StockCode8}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"Excel export hatası: {ex.Message}";
        ...
        private async Task LoadDatasheetsAsync(Guid stockCardId, SEStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadPricesAsync(Guid stockCardId, SEStockCardDetailViewModel viewModel)
        {
            try
            {
        ...
        private async Task LoadInventoryAsync(Guid stockCardId, SEStockCardDetailViewModel viewModel)
        {
            try
            {
